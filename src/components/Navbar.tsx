...
const serviceLinks = [
  ...
  { to: "/curb-gutter", label: "Curb & Gutter" },
      ...
      {mobileOpen && (
        <div className="absolute top-full left-0 right-0 bg-darker/98 backdrop-blur-xl lg:hidden max-h-[80vh] overflow-y-auto" style={{ borderBottom: "1px solid hsl(var(--concrete) / 0.08)" }}>
          <div className="p-4 space-y-1">
            <div className="text-[0.65rem] tracking-[0.14em] uppercase text-orange font-bold mb-2">Services</div>
            {serviceLinks.map(l => <Link key={l.to} to={l.to} className="block py-1.5 text-sm text-muted-text hover:text-concrete no-underline">{l.label}</Link>)}
